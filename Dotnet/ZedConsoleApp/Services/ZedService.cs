...
public class ZedService(
    ...
    public async Task Run()
    {
        ...
        init_params.coordinateSystem = COORDINATE_SYSTEM.RIGHT_HANDED_Z_UP_X_FWD;
        ...
        while (true)
        {
            if (zedCamera.Grab(ref runtimeParameters) == ERROR_CODE.SUCCESS)
            {
                ...
                if (err != ERROR_CODE.SUCCESS)
                {
                    logger.LogError("ERROR in Retrieve Objects: {error}", err);
                }
                ...
                if (i % 2 == 0)
                {
                    ...
                    for (int p = 0; p < objects.numObject; p++)
                    {
                        ...
                        var label = objects.objectData[p].label;
                        var trackingState = objects.objectData[p].objectTrackingState;
                        if (label == OBJECT_CLASS.PERSON && trackingState == OBJECT_TRACKING_STATE.OK)
                        {
                            var position = objects.objectData[p].position;
                            var x = position.X;
                            var y = position.Y;
                            var z = position.Z;
                            logger.LogInformation("Object found at position: {x}, {y}, {z}", x, y, z);

                            try
                            {
                                await mqttClientService.PublishPosition(position);
                            } catch (Exception ex)
                            {
                                logger.LogError(ex, "Failed to publish position.");
                            }
